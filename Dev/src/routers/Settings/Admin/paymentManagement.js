...
class splitTransactionManagement extends Component {
  constructor(props) {
    super(props);
    ...
    this.state = {
      ...
      merchantTransactionsDataCSV: [],
      hdnCustomerSearch: false
    };
  }
  showDrawerFilter = () => {
  ...
  handleGetMerchantTransactionsList(pagination, sorter) {
    ...
    axios({
      ...
      .then(function (res) {
        ...
        self.handleGetMerchantTransactionsCSV(pagination, sorter);
      })
      .catch((data) => {
        console.log(data);
      });
  ...
  handleGetMerchantTransactionsCSV(pagination, sorter) {
    let self = this;
    var paging = pagination !== undefined ? pagination : this.state.paginationMerchant;
    var startDate = this.state.startDate !== null ? moment(this.state.startDate).format('YYYY-MM-DD') : this.state.startDate;
    var endDate = this.state.endDate !== null ? moment(this.state.endDate).format('YYYY-MM-DD') : this.state.endDate;
    axios({
      ...
      data: {
        ...
        SortBy: sorter !== undefined ? (sorter.order !== undefined ? (sorter.order == "ascend" ? "asc" : "desc") : "desc") : "desc",
        Page: "1",
        Size: (this.state.paginationMerchant.total).toString()
      },
    })
      .then(function (res) {
        ...
        if (status === "Success") {
          if (data.length !== 0) {
            self.setState({
              merchantTransactionsDataCSV: data
            });
          } else {
            self.setState({
              merchantTransactionsDataCSV: []
  ...
  handleMerchantDetails(merchantId, orderId) {
    ...
    const columnsMerchant = [
      ...
      },
      {
        ...
        render: (row, item) => (
          <span
            className="custom-link"
            onClick={() => this.handleMerchantDetails(item.merchantId, item.orderId)}
          >
            {item.merchantName}
          </span>
        ),
        ...
        <div className="exfilter">
          {this.state.activeTab == "customer" ? (
            ...
            Export to CSV
              ...
              data={this.state.merchantTransactionsDataCSV}
          ...
          <div className="row m-0 w-100 back">
            ...
            <div className="col-12 col-md-12">
              <div className="search">
                <button onClick={this.state.hdnCustomerSearch == true?this.handleSearchMerchant.bind(this):this.handleSearchCustomer.bind(this)}
...
export default splitTransactionManagement;
