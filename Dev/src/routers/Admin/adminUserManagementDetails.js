...
class adminUserManagementDetails extends Component {
  ...
  componentDidMount() {
    ...
    if (document.getElementById("endCustomerMmt")) {
      document.getElementById("endCustomerMmt").classList.add("active");
  ...
  handleAdminApproveRefund(orderId, TransactionId) {
    
    let self = this;
    axios({
      method: "post",
      url: config.apiUrl + "Transaction/adminApproveRefund",
      headers: authHeader(),
      data: {
        OrderId: orderId,
      ...
      .then(function (res) {
        ...
        let status = res.data.message;
        if (status === "Success") {
          NotificationManager.success("Record successfully approved.");
          self.hide(
            this,
            "customer_" + TransactionId
          )
          self.handleGetUserManagementDetails(self.state.user_Id, self.state.search)
          // self.setState({
          //   userData: data,
          //   userTransactionDetails: data.userTransactions,
          // });
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (row, item) => {
          return (
            <label>{!item.isRefundSuccess ? moment(item.userTransactionDate).format('DD-MM-YYYY') : "-"}</label>
          );
        },
      ...
      <div className="usermanadetail">
        ...
        <div className="row usdetailtext">
          ...
          <div className="col-12 col-md-4">
            ...
            <label></label>
            {/* // userDetails.customerCard.toLowerCase().trim().includes("visa") ? (
                //   <img className="visaind" src={visa} alt="visa" />
                // ) : userDetails.customerCard.toLowerCase().trim().includes("master card") ? (
                //   <img className="visaind" src={mastercard} alt="visa" />
                // ) : userDetails.customerCard.toLowerCase().trim().includes("american express") ? (
                //   <img className="visaind" src={americanexpress} alt="visa" />
                // ) : userDetails.customerCard.toLowerCase().trim().includes("diner club") ? (
