...
import { merchantAuthHeader } from "./merchantAuthHeader";
...
class merchantLogin extends Component {
  ...
  handleFinaleSubmit(e) {
    ...
    if (userName && password) {
      ...
      axios({
        method: "get",
        url: config.apiUrl + "Account/merchantLogin",
        headers: {
        ...
        .then(function (res) {
          ...
          if (resValid === "Valid Login") {
            ...
            window.localStorage.setItem("instantmerchanttoken", res.data.responseData.token);
            // setTimeout(function () {
            ...
            cookies.set("instantmerchanttoken", res.data.responseData.token, { path: "/", maxAge: 86400 });
  ...
  handleGetRolesPermissionsList() {
    ...
    axios({
      method: "get",
      url: config.apiUrl + "MerchantSetting/merchantrolepermissionsbyid",
        ...
        if (status === "Success") {
          // data.map((item) => {
            // if (item.roleName == "Admin") {
              window.localStorage.setItem("InstantPayMerchantModule", data.mappedModuleName);
              window.localStorage.setItem("InstantPayMerchantSubModule", data.mappedSubModuleName);
          ...
          // if (search = "Admin") {
            if(window.localStorage.getItem("InstantPayMerchantModule").includes('Dashboard'))
            {
              setTimeout(function () {
                self.props.history.push("/instantPayMerchant/dashboard");
              }, 100);  
            } else if(window.localStorage.getItem("InstantPayMerchantModule").includes('Transaction History')){
              setTimeout(function () {
                self.props.history.push("/instantPayMerchant/transaction-history");
              }, 100); 
            } else if(window.localStorage.getItem("InstantPayMerchantModule").includes('Subscription')){
              setTimeout(function () {
                self.props.history.push("/instantPayMerchant/merchantSubscription");
              }, 100); 
            } else if(window.localStorage.getItem("InstantPayMerchantModule").includes('Settings')){
              setTimeout(function () {
                self.props.history.push("/instantPayMerchant/merchantSetting");
              }, 100); 
            }
          ...
          <div className="sign-in-card">
            ...
            <form name="sign-in-form" onSubmit={this.handleFinaleSubmit}>
              ...
              <div className="flex-parted">
                <span></span>
                <Link
                  to="instantPayMerchantForgotPassword"
