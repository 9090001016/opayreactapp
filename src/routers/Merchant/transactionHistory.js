...
class transactionHistory extends Component {
  ...
  handleMerchantRefund(orderId, merchantOrderId, transactionID) {
    ...
    axios({
      method: "post",
      url: config.apiUrl + "MerchantTransaction/merchanrapporveRefund",
      headers: merchantAuthHeader(),
      data: {
        OrderId: orderId,
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (row, item) => {
          return(
            <div>
              <label>{!item.isRefundColor ? item.transactionDate: "-"}</label>
            </div>
          );
        },
        sorter: true,
      ...
      },
      {
        ...
        render: (row, item) => {
          return(
            <div>
              <label>{item.isRefundColor ? item.refundDate: "-"}</label>
            </div>
          );
        },
        sortDirections: ['ascend', 'descend', 'ascend']
      ...
      },
      {
        ...
        render: (row, item) => {
          return ((item.isMerchantRefundinitiated == false &&
            ...
            item.isRefundSuccess == false) ? "Refund Failed" : "Refund Success")
        }
      }
    ];
        ...
        <div className="MerchantrHistorytable">
          <Spin spinning={this.state.loading}>
            <Table
              ...
              rowClassName={(record) =>  record.isRefundColor ? 'ant-table-row active-row1' : 'ant-table-row' }
              columns={columns}
