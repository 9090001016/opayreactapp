...
class splitForgotPassword extends Component {
  ...
  handleFinaleSubmit = (e) => {
    ...
    if (this.state.emailId) {
      ...
      axios({
        method: "post",
        url: config.apiUrl + "OnePayUserAccount/ForgetPassword",
        headers: {
  ...
  handleOTPSubmit() {
    ...
    axios({
      method: "post",
      url: config.apiUrl + "OnePayUserAccount/customeremailotpverification",
      ...
      .then(function (res) {
        ...
        if (status === "Success") {
          ...
          window.location.href = `/onePayUserResetPassword`;
        } else {
