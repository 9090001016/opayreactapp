...
class splitForgotPassword extends Component {
  constructor(props) {
    super(props);
  ...
  handleFinaleSubmit = (e) => {
    ...
    if (this.state.emailId) {
      ...
      axios({
        method: "post",
        url: config.apiUrl + "OnePayAccount/ForgetPassword",
        headers: {
  ...
  handleOTPSubmit() {
    ...
    axios({
      method: "post",
      url: config.apiUrl + "OnePayAccount/merchantemailotpverification",
      headers: merchantAuthHeader("no"),
      data: {
        OTP: this.state.otp,
        EmailId: this.state.emailId,
      },
      ...
      .then(function (res) {
        ...
        if (status === "Success") {
          ...
          window.location.href = `/onePayMerchantResetPassword`;
        } else {
